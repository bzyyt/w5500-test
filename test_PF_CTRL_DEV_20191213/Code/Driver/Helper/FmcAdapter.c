...
void FMC_Init()
{
	...
	NORSRAMTimingStructure.FMC_DataSetupTime = 15;
...
void FMC_ReadBuffer(u32 addr, u16 *pBuffer, u32 size)
{
	...
	for (; size != 0; size--)
	{
		*pBuffer++ = *(__IO u16*) (FPGA_BASE_ADDR + ADDR_SHIFT(pointer));
		pointer++;
	}
}


void FMC_WriteBuffer(u32 addr, u16 *pBuffer, u32 size)
{
	__IO u32 pointer = (u32)addr;

	for (; size != 0; size--)
	{
		*(u16*)(FPGA_BASE_ADDR + ADDR_SHIFT(pointer)) = *pBuffer++;
		pointer++;
	}
}

u16 FMC_ReadWord(u32 addr)
{
	return (*(__IO u16*)(FPGA_BASE_ADDR + ADDR_SHIFT(addr)));
}

void FMC_WriteWord(u32 addr, u16 data)
{
	*(u16*)(FPGA_BASE_ADDR + ADDR_SHIFT(addr)) = data;
}
